...
const AvailabilityCalendar = ({ yachts = sampleYachts }) => {
  ...
  const handlegetBlocks = async (selectedYacht) => {
    ...
    const blocks = await dispatch(fetchBlocks(selectedYacht.value));
    ...
    data.forEach((entry) => {
      if (entry.type === "day") {
        ...
        events.push({
          ...
          start: new Date(entry.date), // Start of the day
          end: new Date(entry.date), // End of the day
        ...
        entry.slots.forEach((slot) => {
          events.push({
            ...
            end: new Date(entry.date), // End datetime
          });
        });
      }
    });
  ...
  return (
    <CalendarWrapper>
      ...
      <Modal
        ...
        center
        styles={{ modal: { borderRadius: "20px", padding: "20px" } }}
      >
        <DeleteModalContent>
  ...
  <ModalContent>
      ...
      {selectedYacht ? (
        <div>
          <button
            ...
            style={{ cursor: "pointer", background: "none", border: "none", textDecoration:"none" }}
          >
            <FaArrowLeft /> Back to Yacht Selection
          </button>
          <span style={{fontSize: "1.5rem"}}> | Lock Booking for {selectedDate?.toDateString()}</span>
        </div>
      ) : (
    ...
    <div className="block-modal-content">
      {!selectedYacht ? (
        ...
        <>
          <div className="selected-yacht block-yacht-info" style={{justifyContent: "flex-start", display: "flex", flexDirection: "row"}}>
            <img
