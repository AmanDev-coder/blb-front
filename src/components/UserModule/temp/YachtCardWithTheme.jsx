...
import { Users, Ruler, Star, MapPin, Calendar, Waves, ChevronRight } from "lucide-react";
...
const CardContainer = styled(motion.div)`
  ...
  box-shadow: 
    0 10px 25px rgba(0, 0, 0, ${props => props.isDarkMode ? '0.2' : '0.03'}),
    0 20px 48px rgba(0, 0, 0, ${props => props.isDarkMode ? '0.4' : '0.07'}),
    0 0 0 1px rgba(${props => props.isDarkMode ? '100, 116, 139' : '220, 230, 245'}, 0.5);
  transition: all 0.5s cubic-bezier(0.23, 1, 0.32, 1);
  
  &:hover {
    transform: translateY(-16px) scale(1.02);
    box-shadow: 
      0 20px 40px rgba(0, 0, 0, ${props => props.isDarkMode ? '0.4' : '0.08'}),
      0 15px 25px rgba(59, 130, 246, ${props => props.isDarkMode ? '0.2' : '0.08'}),
      0 0 0 1px rgba(59, 130, 246, ${props => props.isDarkMode ? '0.3' : '0.2'});
  }
`;
...
const LoadingSkeleton = styled.div`
  ...
  background: ${props => props.isDarkMode 
    ? 'linear-gradient(110deg, #1e293b 30%, #334155 50%, #1e293b 70%)' 
    : 'linear-gradient(110deg, #eff6ff 30%, #f1f5f9 50%, #eff6ff 70%)'};
...
const RatingDisplay = styled.div`
  ...
  align-items: center;
  gap: 4px;
  background: ${props => props.isDarkMode ? '#334155' : '#fff'};
  padding: 6px 12px;
  border-radius: 30px;
  box-shadow: 0 4px 12px rgba(0, 0, 0, ${props => props.isDarkMode ? '0.3' : '0.08'});
  ...
  span {
    font-weight: 600;
    color: ${props => props.isDarkMode ? '#e2e8f0' : '#1e3a8a'};
    font-size: 0.95rem;
  }
`;
...
const Feature = styled.div`
  ...
  &:hover {
    color: ${props => props.isDarkMode ? '#f8fafc' : '#1e3a8a'};
...
const BottomSection = styled.div`
  ...
  border-top: 1px solid ${props => props.isDarkMode ? '#334155' : '#e2e8f0'};
  padding: 1.25rem;
  display: flex;
  align-items: center;
...
const ViewButton = styled(motion.div)`
  ...
  font-weight: 600;
  font-size: 0.95rem;
  box-shadow: 0 4px 12px rgba(59, 130, 246, ${props => props.isDarkMode ? '0.4' : '0.3'});
  transition: all 0.3s ease;
  ...
  &:hover {
    box-shadow: 0 6px 16px rgba(59, 130, 246, ${props => props.isDarkMode ? '0.5' : '0.4'});
...
const HeartButton = styled(motion.div)`
  ...
  height: 42px;
  border-radius: 50%;
  background: ${props => props.isDarkMode ? 'rgba(30, 41, 59, 0.9)' : 'rgba(255, 255, 255, 0.9)'};
  ...
  cursor: pointer;
  z-index: 10;
  box-shadow: 0 2px 8px rgba(0, 0, 0, ${props => props.isDarkMode ? '0.3' : '0.15'});
  transition: all 0.3s ease;
...
const CompareCheckbox = styled(motion.div)`
  ...
  border-radius: 30px;
  z-index: 10;
  box-shadow: 0 4px 12px rgba(0, 0, 0, ${props => props.isDarkMode ? '0.3' : '0.1'});
...
const LuxuryTag = styled(motion.div)`
  ...
  box-shadow: 0 4px 12px rgba(0, 0, 0, ${props => props.isDarkMode ? '0.3' : '0.2'});
  overflow: hidden;
    ...
    >
      <ImageContainer>
        ...
        {yacht?.images?.length > 0 ? (
          ...
          <LoadingSkeleton isDarkMode={isDarkMode}>No image available</LoadingSkeleton>
        )}
        ...
        >
          <GoHeartFill 
            size={22} 
            color={isWishlisted ? "#f9274d" : isDarkMode ? "#94a3b8" : "#64748b"} 
          />
        </HeartButton>
      ...
      <ContentContainer>
        ...
        <RatingDisplay isDarkMode={isDarkMode}>
          <Star size={16} fill="#f59e0b" strokeWidth={0} />
          <span>{yacht?.averageRating || "4.5"}</span>
        </RatingDisplay>
        
        {/* Yacht name and location */}
        <YachtName isDarkMode={isDarkMode}>{yacht?.title || "Unnamed Yacht"}</YachtName>
      ...
      <BottomSection isDarkMode={isDarkMode}>
        <PriceDisplay isDarkMode={isDarkMode}>
